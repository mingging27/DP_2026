...
public class SafeFrame extends Frame implements ActionListener, Context {
    ...
    public SafeFrame(String title) {
        ...
        // 패널에 버튼 저장
        Panel panel = new Panel();  // 패널을 이용하여 버튼들을 하나로 묶는다.
        panel.add(buttonUse);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // 어떤 버튼이 눌렸는지 판단 (e.getSource())
        if (e.getSource() == buttonUse) {		// 금고 사용 버튼
            state.doUse(this);  // 현재 상태 객체에게 위임 -> SafeFrame 객체 전달
    ...
    public void setClock(int hour) {
        String clockstring = String.format("현재 시간은 %02d:00", hour);    // 스트링 포맷: 2자리로 표현하되, 한자리 수는 0으로 채운다.
